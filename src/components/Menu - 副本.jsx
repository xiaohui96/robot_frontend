...
class _Menu extends React.Component {
  constructor(props){
    super(props);
   // if (languageType =='en-US'){//nameCN  nameEN
    //从props.menuList中解析出path和name然后分别赋值给新的const对象rootPath和rootName
   // const rootPath=undefined, rootName=undefined;
      const { path: rootPath, nameEN: rootName } = props.menuList;
      console.log(props.menuList);
    //else const { path: rootPath, name: rootName } = props.menuList;
    //}
    //else {
     // const { path: rootPath, nameCN: rootName } = props.menuList;
    //}
    ...
    return this.props.menuList.children.map( menuItem => {
      ...
      if(menuItem.children) {
        menuChildren = menuItem.children.map( item => {
          if (languageType == 'en-US') {
            return (<Menu.Item key={item.path}><span>{item.nameEN}</span></Menu.Item>);
          }
          else return (<Menu.Item key={item.path}><span>{item.nameCN}</span></Menu.Item>);

        });
        if (languageType == 'en-US'){
        return (
          <SubMenu key={menuItem.path} title={<span><Icon iconid={menuItem.icon} /><span className="min-hidden">{menuItem.nameEN}</span></span>}>
            {menuChildren}
          </SubMenu>
        )
        }
          ...
          return (
              <SubMenu key={menuItem.path} title={<span><Icon iconid={menuItem.icon} /><span className="min-hidden">{menuItem.nameCN}</span></span>}>
                {menuChildren}
              </SubMenu>
          )
        }
