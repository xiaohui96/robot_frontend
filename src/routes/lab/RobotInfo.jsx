...
//
class RobotInfo extends Reflux.Component {
    constructor(props) {
        super(props);
        this.store = robotInfoStore;
        this.storeKeys = ['RobotInfoList'];
        this.state = {
            RobotInfoList:[],
            filteredInfo: {},
            selectedRows:[],
    ...
    bulkDeletion = () => {
        ...
        Modal.confirm({
            title: `确定批量删除 ${this.state.selectedRows.length} 个机器人吗?`,
            okText: "确定",
            cancelText: "取消",
            onOk() {
                return new Promise((resolve,reject)=>{
                    AppActions.RobotInfo.retrieve(selectedRows, () => resolve());
                })
            }
        });
    }
    ...
    render() {
        const {RobotInfoList,  filteredInfo, selectedRows} = this.state;
        return (
            <Card title={intl.get('Robot Info')}>
                <FilterForm handleFormChange={this.handleFormChange}/>
                {
                    selectedRows.length > 0 && (
                        <span className="tableListOperator">
              <Button onClick={this.bulkDeletion}>生成机器人报表</Button>
            </span>
                    )
                }
                <Button type="primary" className="add-new" onClick={this.props.onAddMap}>新建机器人</Button>
                <Table
                showHeader={true}
                rowSelection={{
                    ...
                    <Col lg={8} md={12} sm={24}>
                        <FormItem label=" 巡检类型">
                            {getFieldDecorator('name')(
                                <Input placeholder="请输入要查询的机器人名称" />
                            )}
                        </FormItem>
                    </Col>
                    { !formSimple ? null :
...
@Form.create()
class RobotInfoModal extends React.Component {
    constructor(props){
        super(props);
        this.state = {
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFields((err, formData) => {
            if (!err) {
                ...
                if (modalFormData.id) {
                    AppActions.RobotInfo.update(formData, callback);
                } else {
                    AppActions.RobotInfo.create(formData, callback)
                }
            }
        });
    }
    ...
    render() {
        ...
        return (
            <div>
                ...
                <Modal
                    visible={visible}
                    title={modalFormData ?.id? "机器人信息编辑" : "新建机器人"}
                    maskClosable={true}
...
export default withModal(AppActions.RobotInfo, RobotInfoModal)(RobotInfo);
