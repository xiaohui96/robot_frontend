...
//
class TaskDisplay extends Reflux.Component {
    constructor(props) {
        super(props);
        this.store = alarmStore;
        this.storeKeys = ['TasksList'];
        this.state = {
            TasksList:[],
            filteredInfo: {},
            selectedRows:[],
    ...
    bulkDeletion = () => {
        ...
        Modal.confirm({
            title: `确定批量删除 ${this.state.selectedRows.length} 个巡检任务吗?`,
            okText: "确定",
            cancelText: "取消",
            onOk() {
                return new Promise((resolve,reject)=>{
                    AppActions.Alarm.taskretrieve(selectedRows, () => resolve());
                })
            }
        });
    }
    ...
    render() {
        const {TasksList, filteredInfo, selectedRows} = this.state;
        return (
            ...
            >
                    ...
                    <Column  title={intl.get('task name')}
                             dataIndex="taskName"
                             filteredValue= {filteredInfo.taskName ? [filteredInfo.taskName] : null}
                ...
                <Column
                    ...
                    render={(record) => (
                        <div className="table-operate">
                            ...
                            <Tooltip title="删除">
                                <a onClick={()=> this.props.onDelTask(record.taskName, record.id)}>
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFields((err, formData) => {
            if (!err) {
                ...
                if (modalFormData.id) {
                    AppActions.Alarm.taskupdate(formData, callback);
                    //console.log("传递数据:");
                    //console.log(formData);
                } else {
                    AppActions.Alarm.taskupdate(formData, this.props.selectedRows, callback)
                }
            }
        });
    }
...

                        {
                            modalFormData && (
                                <>
                                    ...
                                    <FormItem    label="变电站序号"  {...formItemLayoutInModal}>
                                        {/*<span className="ant-form-text">{modalFormData.account}</span>*/}
                                        {getFieldDecorator('stationId', {
                                            initialValue: modalFormData.stationId,
                                        })(
                                            <span>{ modalFormData.stationId}</span>
                                        )}
                                    </FormItem>

                                    <FormItem   label="任务名称"  {...formItemLayoutInModal} >
                                        {/*<span className="ant-form-text">{modalFormData.realName}</span>*/}
                                        {getFieldDecorator('taskName', {
                                            initialValue: modalFormData.taskName,
                                        })(
                                            (
                                                <Input />
                                            )
...
export default withModal(AppActions.Alarm, TaskDisplayModal)(TaskDisplay);
